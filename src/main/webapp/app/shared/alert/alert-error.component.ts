...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: JhiAlertService, private eventManager: JhiEventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('testModelSimpleApp.error', (response: JhiEventWithContent<AlertError>) => {
      const errorResponse = response.content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('testModelSimpleApp.httpError', (response: JhiEventWithContent<HttpErrorResponse>) => {
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName = translateService.instant('testModelSimpleApp.' + fieldError.objectName + '.' + convertedField);
