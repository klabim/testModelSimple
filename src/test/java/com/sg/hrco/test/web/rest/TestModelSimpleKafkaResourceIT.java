...
import com.sg.hrco.test.config.KafkaProperties;
...
class TestModelSimpleKafkaResourceIT {
    ...
    void setup() {
        ...
        TestModelSimpleKafkaResource kafkaResource = new TestModelSimpleKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/test-model-simple-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/test-model-simple-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
